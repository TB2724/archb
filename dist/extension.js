...
var DiffAnalyzer = class {
   ...
   */
  static extract(oldContent, newContent) {
   ...
   */
  static onlyCommentChanges(added, removed) {
  ...
  static classify(oldContent, newContent) {
    ...
    const stripComments = (s) => s.replace(/\/\/.*/g, "").replace(/\/\*[\s\S]*?\*\//g, "");
    ...
    const layoutCandidate = meaningful.replace(/^\s*import\s.*$/gm, "").replace(/\s+/g, " ").trim();
  ...
  static summarize(oldContent, newContent) {
    const { added, removed } = this.extract(oldContent, newContent);
  ...
  static buildContext(filePath, summary) {
...
var FileWatcher = class {
  ...
  stop() {
    ...
    this.disposables = [];
    this.watcher = void 0;
    this.pendingChanges.forEach((pc) => {
      if (pc.timer) {
  ...
  preloadFiles(dir) {
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && this.IGNORED_DIRS.has(entry.name)) {
          continue;
        }
        if (!entry.isDirectory() && this.IGNORED_FILES.has(entry.name)) {
          continue;
        }
  ...
  onDocumentChanged(doc) {
    ...
    if (existing) {
      ...
      if (existing.oldContent.replace(/\s/g, "") === newContent.replace(/\s/g, "")) {
        return;
      }
      clearTimeout(existing.timer);
      ...
      if (oldContent.replace(/\s/g, "") === newContent.replace(/\s/g, "")) {
        return;
      }
...
var StateManager = class {
  ACTIVE_KEY = "archb.isActive";
  GIT_KEY = "archb.gitSettings";
  context;
  constructor(context) {
    this.context = context;
  }
  isActive() {
    return this.context.globalState.get(this.ACTIVE_KEY, false);
  }
  ...
  getGitSettings() {
    return this.context.globalState.get(this.GIT_KEY, {
      userName: "",
      userEmail: "",
      remoteUrl: "",
      branch: "main",
...
var OllamaClient = class {
  ...
  QUESTION_TIMEOUT = 3e4;
  LOGBOOK_TIMEOUT = 6e4;
  CLUSTER_TIMEOUT = 2e4;
  COMMIT_MSG_TIMEOUT = 2e4;
  stateManager;
  ...
  async generateQuestion(codeContext, changeType) {
   ...
   */
  async clusterChanges(descriptions) {
    if (descriptions.length <= 1) {
      return descriptions.map((_2, i) => [i]);
    }
    const numbered = descriptions.map((d, i) => `[${i}] ${d}`).join("\n---\n");
    const prompt = `Given these code changes, group them by whether they belong to the same logical modification (same feature, same refactor, same bugfix). Output ONLY a JSON array of arrays of indices, nothing else. Example: [[0,2],[1],[3,4]]
...
${numbered}`;
  ...
  async generateCommitMessage(entries) {
    const answered = entries.filter((e) => e.question && e.answer);
    if (answered.length === 0) {
      return "Archb: auto-commit";
    }
    const summary = answered.map((e) => `${e.file.split(/[/\\]/).pop()}: ${e.answer}`).join("\n");
    try {
      const raw = await this.chat(
        [
          {
...
Write the commit message.`
  ...
  async generateLogbook(entries, type, version) {
    const headerLine = version ? `# ${this.headerFor(type)} \u2014 ${version}` : `# ${this.headerFor(type)}`;
    if (entries.length === 0) {
      return `${headerLine}
...
`;
    ...
    if (type === "qna") {
      const sections = entries.filter((e) => e.question).map((e) => {
...
`;
    ...
    const sectionsByFile = /* @__PURE__ */ new Map();
    for (const e of entries) {
...
Developer note: ${e.answer}`;
      const systemMsg = type === "technical" ? "You are a changelog assistant. Given a code diff and a developer comment, generate a clear professional changelog entry in 1-2 sentences in past tense. Explain WHAT changed and WHY." : "You are a release-notes writer. Given a code diff and a developer comment, write ONE customer-friendly sentence in past tense describing what changed from the user perspective. Avoid technical jargon, file names, or function names. Output only the sentence.";
      let line;
      try {
        const raw = await this.chat(
          [
        ...
        if (!line) {
          line = type === "technical" ? `Updated \`${fileName}\` (${e.changeType}). ${e.answer}` : `${e.answer}`;
        }
      } catch {
        line = type === "technical" ? `Updated \`${fileName}\` (${e.changeType}). ${e.answer}` : `${e.answer}`;
      }
...
`;
    ...
    let totalEntries = 0;
    for (const arr of sectionsByFile.values()) {
  ...
  async chat(messages, options, timeoutMs) {
    const mode = this.getMode();
  ...
  async chatOpenAI(messages, options, timeoutMs, model) {
    const settings = this.stateManager.getGitSettings();
    const ctrl = new AbortController();
  ...
  async chatClaude(messages, options, timeoutMs, model) {
    const settings = this.stateManager.getGitSettings();
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), timeoutMs);
    const started = Date.now();
  ...
  async chatOllama(messages, options, timeoutMs) {
  ...
  buildUserPrompt(codeContext, changeType) {
...
Ask one short question about why this change was made.`;
  ...
  changeTypeHint(changeType) {
    switch (changeType) {
      case "NewFunction" /* NewFunction */:
        return "The developer added a new function or method.";
      case "ValueChange" /* ValueChange */:
        return "The developer changed a value, variable, or parameter.";
      case "Logic" /* Logic */:
        return "The developer changed program logic (if/else/loop/condition).";
      case "Layout" /* Layout */:
        return "The developer made formatting or structural changes.";
      case "Comment" /* Comment */:
        return "The developer added or modified a code comment.";
      default:
        return "The developer made a code change.";
    }
  }
  extractQuestion(raw) {
    if (!raw || raw.length < 6) {
  ...
  fallbackQuestion(changeType, context) {
...
${diff.slice(0, 300)}`;
  ...
  deterministicCluster(entries) {
    ...
    for (let i = 0; i < clusters.length; i++) {
      ...
      for (let j = i + 1; j < clusters.length; j++) {
        ...
        if (clusters[j].changeType !== clusters[i].changeType) {
          continue;
        }
  ...
  tokens(diff) {
    return new Set(
      (diff || "").split(/[\s\W]+/).map((t) => t.toLowerCase()).filter((t) => t.length > 2)
    );
  }
  jaccard(a, b) {
  ...
  mergeClusters(parts) {
    ...
    const representative = parts.map((p) => p.representative).slice().sort((a, b) => (b.diff?.length ?? 0) - (a.diff?.length ?? 0))[0];
    return {
      entryIds,
      representative,
...
var OverlayManager = class {
  ...
  mode = "save";
  onComplete;
  constructor(context, log, stateManager2) {
    this.context = context;
    this.log = log;
    this.ollama = new OllamaClient(stateManager2);
    this.clusterer = new Clusterer(this.ollama);
  }
  async startSession(mode, onComplete) {
    this.mode = mode;
    this.onComplete = onComplete;
    const pending = this.log.getPendingChanges();
    const unanswered = this.log.getUnansweredWithQuestion();
    const toProcess = [...pending, ...unanswered];
    console.log(`[Archb] startSession(${mode}): ${toProcess.length} entries to process`);
    if (toProcess.length === 0) {
      vscode2.window.showInformationMessage("Archb: No changes to review.");
      if (this.onComplete) {
    ...
    await vscode2.window.withProgress(
      { location: vscode2.ProgressLocation.Notification, title: "Archb: preparing questions\u2026", cancellable: false },
      async () => {
        ...
        for (const cluster of clusters) {
          ...
          console.log(`[Archb] asking Ollama for ${path2.basename(rep.file)} (${rep.changeType})`);
          const question = await this.ollama.generateQuestion(ctx, rep.changeType);
          console.log(`[Archb] got question: "${question}"`);
          const finalQuestion = question && question.trim().length > 0 ? question : this.defaultQuestion(rep.changeType, rep.file);
          ...
          items.push({
            ...
            codeSnippet: snippet,
            fileName: path2.basename(rep.file),
            prefilledAnswer: ""
          });
        }
        this.items = items;
  ...
  revisitEntry(entryId) {
    const entry = this.log.getEntryById(entryId);
    if (!entry || !entry.question) {
      vscode2.window.showInformationMessage("Archb: No question recorded yet for this entry.");
      return;
    }
    ...
    this.items = [{
      ...
      codeSnippet: snippet,
      fileName: path2.basename(entry.file),
  ...
  showCurrent() {
    ...
    this.panel.webview.html = this.buildHtml(
      ...
      item.codeSnippet,
      item.fileName,
      this.changeTypeLabel(item.cluster.changeType),
      remaining,
      item.prefilledAnswer,
      this.cursor > 0,
      item.cluster.filePaths.length
    );
    this.panel.reveal(vscode2.ViewColumn.Two, false);
  }
  ...
  defaultQuestion(changeType, file) {
    const fileName = path2.basename(file);
    switch (changeType) {
      case "NewFunction" /* NewFunction */:
        return `Why did you add this new function in ${fileName}?`;
      case "ValueChange" /* ValueChange */:
        return `Why did you change this value in ${fileName}?`;
      case "Logic" /* Logic */:
        return `Why did you modify this logic in ${fileName}?`;
      case "Layout" /* Layout */:
        return `Why did you make these structural changes in ${fileName}?`;
      case "Comment" /* Comment */:
        return `What does this new comment in ${fileName} document?`;
      default:
        return `Why did you make this change in ${fileName}?`;
    }
  }
  buildHtml(question, codeSnippet, fileName, changeType, remaining, prefilledAnswer, canGoBack, fileCount) {
    const safeQuestion = question && question.trim().length > 0 ? question : "Why did you make this change?";
...
<div class="hint">Enter = submit &amp; next \xB7 Shift+Enter = newline${canGoBack ? " \xB7 Shift+Tab = back" : ""}</div>
...
var LogManager = class {
  ...
  getActivityPath() {
    ...
    return path3.join(folders[0].uri.fsPath, "activity.json");
  }
  loadFromStorage() {
    const stored = this.context.globalState.get("archb.entries", []);
    this.entries = stored;
    this.logsDir = this.context.globalState.get("archb.currentLogsDir");
    this.currentVersion = this.context.globalState.get("archb.currentVersion", "v1.0");
  }
  persist() {
...
**A:** ${e.answer ?? "(no answer)"}`;
      ...
      await fsp.writeFile(
        path3.join(this.logsDir, `${this.currentVersion}_qna_live.md`),
...
\`\`\``;
      ...
      await fsp.writeFile(
        path3.join(this.logsDir, `${this.currentVersion}_raw_live.md`),
...
${rawLines || "_No entries yet._"}`,
  ...
  flushSync() {
    ...
    if (actPath) {
      try {
        fs2.writeFileSync(actPath, JSON.stringify({ entries: this.entries }, null, 2), "utf-8");
      } catch {
      }
    }
    if (this.logsDir) {
...
${qnaLines || "_No entries yet._"}`,
  ...
  addRawChange(filePath, changeType, diffSummary, oldContent, newContent, capturedAt) {
    const diff = [
  ...
  setQuestionForEntry(entryId, question) {
  ...
  setQuestionForCluster(entryIds, question) {
  ...
  setAnswerForCluster(entryIds, answer) {
    for (const id of entryIds) {
      const entry = this.entries.find((e) => e.id === id);
      if (!entry) {
        continue;
  ...
  async clearEntries() {
    ...
    if (this.logsDir) {
      try {
        const qna = path3.join(this.logsDir, `${this.currentVersion}_qna_live.md`);
        const raw = path3.join(this.logsDir, `${this.currentVersion}_raw_live.md`);
        await fsp.writeFile(qna, `# Q&A Log \u2014 ${this.currentVersion}

_No entries yet._`, "utf-8").catch(() => {
        });
...
var VersionManager = class {
  ...
  ollama;
  currentLogsDir;
  VERSION_KEY = "archb.currentVersion";
  snapshotStatus = "idle";
  onStatusChange;
  constructor(context, stateManager2) {
    this.context = context;
    this.ollama = new OllamaClient(stateManager2);
  }
  setLogManager(logManager2) {
  ...
  getVersions() {
    ...
    const versionsDir = path4.join(folders[0].uri.fsPath, "versions");
    if (!fs3.existsSync(versionsDir)) {
      return [];
    }
    try {
      return fs3.readdirSync(versionsDir, { withFileTypes: true }).filter((e) => e.isDirectory() && /^v\d+\.\d+$/.test(e.name)).map((e) => {
        const metaPath = path4.join(versionsDir, e.name, "meta.json");
        if (fs3.existsSync(metaPath)) {
          try {
            return JSON.parse(fs3.readFileSync(metaPath, "utf-8"));
          } catch {
          }
        }
        const stat = fs3.statSync(path4.join(versionsDir, e.name));
  ...
  getCurrentVersion() {
    return this.context.globalState.get(this.VERSION_KEY, "v1.0");
  }
  /**
  ...
  async createNewSession() {
    ...
    const versionDir = path4.join(rootPath, "versions", version);
    const snapshotDir = path4.join(versionDir, "snapshot");
    const logsDir = path4.join(versionDir, "logs");
    await fsp2.mkdir(snapshotDir, { recursive: true });
    await fsp2.mkdir(logsDir, { recursive: true });
    await fsp2.writeFile(
      path4.join(versionDir, "meta.json"),
      JSON.stringify({ version, timestamp: (/* @__PURE__ */ new Date()).toISOString() }, null, 2)
    );
    ...
    if (this.logManager) {
      this.logManager.setLogsDir(logsDir, version);
    }
    this.setStatus("creating");
    const zipPath = path4.join(snapshotDir, `snapshot.zip`);
    this.createZipSnapshot(rootPath, zipPath).then(() => this.setStatus("ready")).catch((err) => {
  ...
  async finalizeSession() {
    const logsDir = this.currentLogsDir ?? this.context.globalState.get("archb.currentLogsDir");
    if (!logsDir || !this.logManager) {
      return;
    }
    ...
    try {
      ...
      await fsp2.writeFile(path4.join(logsDir, `${version}_technical.md`), technical, "utf-8");
      console.log(`[Archb] wrote ${version}_technical.md`);
      const customer = await this.ollama.generateLogbook(entries, "customer", version);
      await fsp2.writeFile(path4.join(logsDir, `${version}_customer.md`), customer, "utf-8");
  ...
  nextVersion() {
    ...
    for (const v of versions) {
      ...
      if (p.major === major && p.minor > maxMinor) {
        maxMinor = p.minor;
      }
    }
   ...
   */
  createZipSnapshot(sourceDir, outputPath) {
    return new Promise((resolve, reject) => {
...
var GitManager = class {
  ...
  logManager;
  ollama;
  constructor(stateManager2, logManager2) {
    this.stateManager = stateManager2;
    this.logManager = logManager2;
    this.ollama = new OllamaClient(stateManager2);
  }
  ...
  async commitAndPush() {
    ...
    const terminal = vscode5.window.createTerminal("Archb \u2013 GitHub Push");
    terminal.show(true);
  ...
  hasGitDir(cwd) {
    return fs4.existsSync(path5.join(cwd, ".git"));
  }
  /**
...
var SidebarProvider = class {
  ...
  onTriggerSave;
  onTriggerPush;
  constructor(context, stateManager2, logManager2, versionManager2) {
    this.context = context;
    this.stateManager = stateManager2;
    this.logManager = logManager2;
    this.versionManager = versionManager2;
  }
  resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.type) {
        case "toggle":
          await vscode6.commands.executeCommand("archb.toggleActive");
          this.pushData();
          break;
        case "saveSession":
          if (this.onTriggerSave) {
            this.onTriggerSave();
          }
          break;
        case "pushGitHub":
          if (this.onTriggerPush) {
            this.onTriggerPush();
          }
          break;
        case "clearLogs":
        ...
        case "requestData":
          ...
          break;
        case "replayEntry":
          if (this.onReplayEntry && msg.entryId) {
            this.onReplayEntry(msg.entryId);
          }
          break;
        case "saveGitSettings":
          if (msg.settings) {
            await vscode6.commands.executeCommand("archb.openGitSettings", msg.settings);
            this.pushData();
          }
          break;
      }
...
async function activate(context) {
  ...
  sidebarProvider.onReplayEntry = (entryId) => {
  ...
  sidebarProvider.onTriggerPush = async () => {
    ...
    if (!settings.userName.trim() || !settings.userEmail.trim()) {
      vscode7.window.showWarningMessage("Archb: Please set Git user.name and user.email in the sidebar settings first.");
      return;
    }
    await overlayManager.startSession("push", async (completed) => {
      ...
      if (result.ok) {
        vscode7.window.showInformationMessage("Archb: " + result.message);
      } else {
        vscode7.window.showErrorMessage("Archb: " + result.message);
      }
    });
  };
  logManager.setOnChange(() => {
    sidebarProvider.pushData();
  });
  versionManager.setOnStatusChange((status) => {
    sidebarProvider.onSnapshotStatus(status);
  });
  context.subscriptions.push(
    vscode7.window.registerWebviewViewProvider("archb.sidebarView", sidebarProvider, {
      webviewOptions: { retainContextWhenHidden: true }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("archb.toggleActive", async () => {
      const isActive = await stateManager.toggleActive();
      if (isActive) {
        await startWatching();
        vscode7.window.showInformationMessage("Archb is now ACTIVE");
      } else {
        await stopWatching();
        vscode7.window.showInformationMessage("Archb is now INACTIVE");
      }
      sidebarProvider.pushData();
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("archb.openPanel", () => {
      vscode7.commands.executeCommand("workbench.view.extension.archb-sidebar");
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("archb.pushToGithub", async () => {
      if (sidebarProvider?.onTriggerPush) {
        await sidebarProvider.onTriggerPush();
      }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("archb.saveSession", async () => {
      if (sidebarProvider?.onTriggerSave) {
        await sidebarProvider.onTriggerSave();
      }
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("archb.openGitSettings", async (settings) => {
      if (settings && gitManager) {
        const tokenError = validateApiTokens(settings);
        if (tokenError) {
          vscode7.window.showErrorMessage("Archb: " + tokenError);
          return;
        }
        const result = await gitManager.saveSettings(settings);
        if (result.ok) {
          const mode = getActiveAiMode(settings);
          const modeMsg = mode === "claude" ? "Claude API active." : mode === "chatgpt" ? "ChatGPT API active." : "No API key set \u2014 using Ollama.";
          vscode7.window.showInformationMessage(`Archb: ${result.message} ${modeMsg}`);
        } else {
          vscode7.window.showErrorMessage("Archb: " + result.message);
        }
        sidebarProvider.pushData();
...
async function startWatching() {
  ...
  if (!workspaceFolders || workspaceFolders.length === 0) {
    vscode7.window.showWarningMessage("Archb: No workspace folder open.");
...
function validateApiTokens(settings) {
...
function getActiveAiMode(settings) {
  const claude = settings.claudeToken?.trim() ?? "";
  const chatgpt = settings.chatgptToken?.trim() ?? "";
  if (claude.startsWith("sk-ant-")) {
    return "claude";
